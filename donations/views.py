            ...
            if data['frequency'] == "one-time":
                ...
                if exists['status']:
                    ...
                    if len(cus_subs) > 0:
                        ...
                        if res2['status']:
                            return Response({"message": "Donation received successfully"}, status=status.HTTP_200_OK)
                        ...
                        r = requests.post(
                            init_url, headers=headers, data=donation_data)
                        res = r.json()
                        return Response({"url": res['data']['authorization_url']}, status=status.HTTP_200_OK)
